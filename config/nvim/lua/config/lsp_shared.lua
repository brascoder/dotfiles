...
local function with_float_opts(config)
  return vim.tbl_extend("force", config or {}, M.float_opts)
end

local hover_handler = vim.lsp.handlers["textDocument/hover"]
vim.lsp.handlers["textDocument/hover"] = function(err, result, ctx, config)
  return hover_handler(err, result, ctx, with_float_opts(config))
end

local signature_handler = vim.lsp.handlers["textDocument/signatureHelp"]
vim.lsp.handlers["textDocument/signatureHelp"] = function(err, result, ctx, config)
  return signature_handler(err, result, ctx, with_float_opts(config))
end

vim.diagnostic.config({
  float = vim.tbl_extend("force", M.float_opts, {
    source = "always",
  }),
})
